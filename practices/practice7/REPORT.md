...
## 4. Блок-схема алгоритма (Flowchart)

```mermaid
graph TD
    Start([Запуск программы]) --> Init[Инициализация данных N]
    Init --> Malloc[Выделение памяти на GPU]
    Malloc --> H2D[Копирование данных Host -> Device]

    subgraph Reduction [Редукция]
        R_Start[Начало редукции] --> R_Grid[Расчет сетки: GridSize = N / BlockSize]
        R_Grid --> R_Kernel[Запуск ядра reduce_shared]
        
        subgraph ReduceKernel [Kernel: reduce_shared]
            RK_Load[Загрузка в Shared Memory] --> RK_Sync1[__syncthreads]
            RK_Sync1 --> RK_Loop{Stride > 0?}
            RK_Loop -- Да --> RK_Add[sdata id += sdata id + stride]
            RK_Add --> RK_Sync2[__syncthreads]
            RK_Sync2 --> RK_NextStride[Stride /= 2]
            RK_NextStride --> RK_Loop
            RK_Loop -- Нет --> RK_Write[Запись частичной суммы блока в Global Mem]
        end
        
        R_Kernel --> R_D2H[Копирование частичных сумм на Host]
        R_D2H --> R_CPU[Финальное суммирование на CPU]
    end

    subgraph Scan [Сканирование - Рекурсивное]
        S_Start[Начало сканирования] --> S_Recurse{NumBlocks > 1?}
        
        subgraph ScanBlock [Kernel: scan_block_kernel]
            SK_Load[Загрузка в Shared Memory] --> SK_Hillis{Stride < BlockDim?}
            SK_Hillis -- Да --> SK_Add[Scan Step: temp id += temp id-stride]
            SK_Add --> SK_Sync[__syncthreads]
            SK_Sync --> SK_Next[Stride *= 2]
            SK_Next --> SK_Hillis
            SK_Hillis -- Нет --> SK_Write[Запись результата в Output]
            SK_Write --> SK_Sum[Сохранение суммы блока]
        end

        S_Recurse -- Да --> S_Alloc[Выделение памяти для сумм блоков]
        S_Alloc --> S_RunBlock[Запуск scan_block_kernel]
        S_RunBlock --> S_RecCall[Рекурсивный вызов для массива сумм]
        S_RecCall --> S_Add[Запуск add_block_sum_kernel]
        S_Add --> S_Free[Освобождение временной памяти]
        
        S_Recurse -- Нет --> S_Single[Запуск scan_block_kernel]
    end

    H2D --> Reduction
    Reduction --> Scan
    Scan --> Verify[Сверка результатов с CPU]
