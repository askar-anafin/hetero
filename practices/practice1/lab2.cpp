...
public:
	...
	void print() {
		...
		for (int i = static_cast<int>(data.size()) - 1; i >= 0; --i) {
			cout << data[static_cast<size_t>(i)] << " ";
		}
		cout << "\n";
	}
};
...
public:
	...
	bool dequeue() {
		...
		if (front_idx > data.size() / 2 && front_idx > 100) {
			data.erase(data.begin(), data.begin() + static_cast<std::ptrdiff_t>(front_idx));
	...
	size_t size() {
		lock_guard<mutex> lock(mtx);
		return (data.size() >= front_idx) ? (data.size() - front_idx) : 0;
	}
...
void testStackParallel(int N, int threads) {
	...
	size_t size_seq = stackSeq.size();
	cout << "    Done.\n";
	cout << "    - Elements pushed: " << size_seq << "\n";
	cout << "    - Time: " << dur_seq.count() << " ms\n";
	cout << "    - Rate: " << (static_cast<double>(size_seq) / dur_seq.count()) << " elements/ms\n";
	...
	size_t size_par = stackPar.size();
	cout << "    Done.\n";
	cout << "    - Elements pushed: " << size_par << "\n";
	cout << "    - Time: " << dur_par.count() << " ms\n";
	cout << "    - Rate: " << (static_cast<double>(size_par) / dur_par.count()) << " elements/ms\n";
